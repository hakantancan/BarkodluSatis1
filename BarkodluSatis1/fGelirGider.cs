...
namespace BarkodluSatis1
{
    public partial class fGelirGider : Form
    {
        ...
        {
            if(cmbOdemeTuru.Text!="")                                              //Ödeme türünün boş olup olmadığını kontrol eder.
            {
                if (tNakit.Text!="" &&tKart.Text!="")                              //Nakit ve kart alanlarının boş olup olmadığını kontrol eder.
                {
                    using (var db=new Database1Entities())                         //Veritabanı bağlantısını oluşturur.
                    {
                        IslemOzet io = new IslemOzet();                            //Yeni bir IslemOzet nesnesi oluşturur. Bu, veritabanına kaydedilecek verileri içerir.
                        io.IslemNo = 0;
                        io.Iade = false;
